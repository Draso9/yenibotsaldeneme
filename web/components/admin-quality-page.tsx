...
function metricLabel(key: string): string {
  const labels: Record<string, string> = {
    python_satir: "Python satırı",
    css_satir: "CSS satırı",
    important: "!important",
    media_query: "Media query",
    hardcoded_hex: "Hardcoded HEX",
    design_token_kullanimi: "Design token kullanımı",
    gecersiz_design_token: "Geçersiz design token",
    "10px_alti_font": "10px altı font",
...
export function AdminQualityPage() {
    ...
    void (async () => {
      try {
        ...
        if (caught instanceof IzfinApiError && caught.status === 403) {
          setForbidden(true);
          setError("Bu ekran yalnızca IZFIN yöneticilerine açıktır.");
          return;
        }
  ...
  return <main className="admin-quality-page" aria-label="IZFIN admin kalite kontrol merkezi">
    <section className="admin-quality-hero">
      ...
      {data && <span className={`admin-quality-status ${data.status.seviye}`}>{data.status.durum}</span>}
    </section>
    ...
    {data && <>
      <section className="admin-quality-kpis">
        {metricEntries.map(([key, value]) => <article className="admin-quality-panel admin-quality-card" key={key}>
      ...
      <section className="admin-quality-grid">
        <article className="admin-quality-panel admin-quality-notes">
          <p className="eyebrow">RELEASE DURUMU</p>
          <h2>{data.status.durum}</h2>
          <ul>{data.status.notlar.map((note) => <li key={note}>{note}</li>)}</ul>
        </article>
        <article className="admin-quality-panel admin-quality-notes">
          <p className="eyebrow">YORUM</p>
          <h2>Teknik borç görünümü</h2>
          <p>Bu ekran mevcut framework-neutral kalite servisini kullanır. Buradaki uyarılar release engeli olmak zorunda değildir; ancak yeni refactor kararlarında önceliklendirme sinyali olarak değerlendirilmelidir.</p>
        </article>
      </section>
    </>}
  </main>;
