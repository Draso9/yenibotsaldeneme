...
function googleAuthErrorMessage(reason: unknown): string {
  switch (firebaseErrorCode(reason)) {
    case "auth/unauthorized-domain":
      return `Google girişi bu alan adı için yetkilendirilmemiş. IZFIN'i https://${STABLE_IZFIN_HOST} üzerinden açın veya destekle iletişime geçin.`;
    case "auth/operation-not-allowed":
      return "Google ile giriş şu anda kullanıma açık değil. E-posta ve şifre ile giriş yapabilir veya destekle iletişime geçebilirsin.";
    ...
    default:
      return "Google ile giriş tamamlanamadı. Biraz sonra tekrar deneyin veya e-posta ve şifre ile giriş yapın.";
  }
}
...
export function AuthPage() {
  ...
  async function register(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError("Hesap oluşturulamadı. E-posta zaten kayıtlı olabilir; bilgilerini kontrol edip tekrar deneyin.");
    } finally { setBusy(false); }
  }
  ...
  return <main className="auth-page"><section className="auth-screen-card"><Link className="auth-logo" href="/"><IzfinBrandMark priority /><span><b>IZFIN</b><small>ANALYZE · PREDICT · INVEST</small></span></Link><p className="eyebrow">SIGNATURE INTELLIGENCE</p><h1>{mode === "login" ? "Hoş Geldiniz" : mode === "register" ? "IZFIN hesabını oluştur" : "Şifreni yenile"}</h1><p className="auth-screen-intro">Piyasayı analiz et, fırsatları filtrele, kararını tek merkezden yönet.</p>
    ...
    <p className="auth-screen-foot">Güvenli oturum · kişisel veri alanı · yatırım karar destek platformu</p>
  </section></main>;
}
