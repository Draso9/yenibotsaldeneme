...
export function ScanWorkspace() {
  ...
  const [symbolDraft, setSymbolDraft] = useState(""); const [job, setJob] = useState<ScanJob | null>(null);
  ...
  return <section className="scan-workspace" aria-label="Akıllı tarama">
    ...
    <div className="scan-control-grid"><section className="scan-control-card"><p className="eyebrow">KİŞİSEL LİSTE</p><h3>Kişisel Listemi Yönet</h3><p>Kalıcı takip listen, Kendi Listem profilinin tarama evrenidir.</p>{watchlist ? <><div className="ticker-list">{watchlist.tickers.map((item) => <button key={item} type="button" onClick={() => removeSymbol(item)} aria-label={`${item} sembolünü listeden sil`}>{item} <span>×</span></button>)}</div><form className="watchlist-form scan-watchlist-form" onSubmit={addSymbol}><label>Sembol ekle<input value={symbolDraft} onChange={(event) => setSymbolDraft(event.target.value)} placeholder="örn. THYAO.IS" /></label><button type="submit">Listeye ekle</button></form></> : <p className="scan-empty">Kişisel liste yükleniyor…</p>}</section>
