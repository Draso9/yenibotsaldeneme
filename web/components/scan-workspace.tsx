...
import { normalizeRecoveredScanJob, preferActiveRecoveryJob, preferRecoveredJob, recoverableJob, recoveryRetryDelayMs } from "../lib/scan-recovery.mjs";
...
export function ScanWorkspace() {
  ...
  const loadRecoveryJobs = useCallback(async () => {
    ...
    try {
      ...
      if (candidate?.status === "queued" || candidate?.status === "running") {
        ...
        const recovered = normalizeRecoveredScanJob(await fetchScanJobContext(candidate.job_id, token)) as ScanJob;
    ...
    catch { setRecoveryDiscoveryFailures((current) => current + 1); }
  }, [getIdToken, publishCompletedScan, recoverActiveJob, user]);
  ...
  useEffect(() => {
    if (!user || recoveryDiscoveryFailures === 0) return;
  ...
  async function submit() {
    ...
    try {
      ...
      if (completed.status === "completed") {
        await publishCompletedScan(completed, universe.tickers);
      }
    }
    catch (caught) {
...
function ScanResult({ jobId, summary, activeFilter, onFilterChange }: Readonly<{ jobId: string; summary: ScanSummary; activeFilter: ResultFilter; onFilterChange: (filter: ResultFilter) => void }>) {
  ...
  useEffect(() => {
    setSelectedTicker((current) => decisionTickers.includes(current) ? current : (decisionTickers[0] ?? ""));
  }, [decisionTickers]);
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch { if (active) setDecisionError("Hisse karar motoru bu sonuç için yüklenemedi."); }
    })();
    return () => { active = false; };
