...
function TechnicalOverview({ technical, fallback }: Readonly<{ technical?: import("../lib/market-center").StructuredTechnicalAnalysis; fallback: Record<string, unknown> }>) {
  ...
  return <article className="detail-section detail-wide detail-technical">
    ...
    <details className="detail-technical-disclosure"><summary>Trend ve momentum</summary><div className="detail-technical-rows">{trend.map((item, index) => <div key={`${text(item.label)}-${index}`}><span>{text(item.label)}</span><b className={`is-${technicalTone(item.tone)}`}>{text(item.value)}</b></div>)}</div></details>
