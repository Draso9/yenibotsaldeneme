...
def test_readiness_requires_scan_job_store_and_durable_persistence():
    missing_store = TestClient(_app_with_store(None)).get("/api/v1/health/ready").json()
    assert missing_store["scan_job_store"] is False
    assert missing_store["scan_job_persistence"] is False
    assert missing_store["ready"] is False
