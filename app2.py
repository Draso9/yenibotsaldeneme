...
IZFIN_APP_SURUMU = "v1.8.26 Native Movers"
...
def izfin_movers_render(max_n=5):
    """BÃ¼yÃ¼k Hareketler'i Markdown/Ã¶zel grid yerine native Streamlit kolonlarÄ±yla Ã§izer."""
    sonuclar = st.session_state.get("sonuclar") or []
    paneller = st.session_state.get("teknik_paneller") or {}
    rows = []
    for sonuc in sonuclar:
    ...
    with st.container(border=True):
        st.html('<div class="iz-card-title">BÃœYÃœK HAREKETLER</div>')
        if not rows:
            st.info("AkÄ±llÄ± Tarama sonrasÄ± bÃ¼yÃ¼k hareketler burada gÃ¶rÃ¼necek.")
            return

        secilenler = rows[:max_n]
        for index, (_, degisim, ticker, fiyat) in enumerate(secilenler):
            ticker_col, fiyat_col, degisim_col = st.columns([1.0, 1.65, 0.62], gap="small")
            ticker_col.markdown(f"**{ticker}**")
            fiyat_col.text(str(fiyat))
            renk = "green" if degisim >= 0 else "red"
            degisim_col.markdown(f":{renk}[**{degisim:+.2f}%**]")
            if index < len(secilenler) - 1:
                st.divider()
