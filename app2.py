...
def izfin_qa_center_render():
    ...
    with st.expander("🎨 Aktif Alım Pozisyonları · Tema Kontrolü", expanded=False):
        ...
        with dolu_tab:
            ...
            st.html(aktif_pozisyon_tablosu_html(qa_aktif_ornek))
        with bos_tab:
...
if aktif_sayfa in ["🏠 Ana Sayfa", "🔎 Akıllı Tarama"]:
    if aktif_sayfa == "🏠 Ana Sayfa":
        ...
        scan_sayfa_html = tarama_sayfa_html_paketi_hazirla(
            len(st.session_state.get("custom_tickers", []))
        )
        st.html(scan_sayfa_html["hero_html"])
        ...
        st.markdown(scan_sayfa_html["control_header_html"], unsafe_allow_html=True)

        scan_left, scan_right = st.columns([1.0, 1.15], gap="large")

        with scan_left:
            st.markdown(scan_sayfa_html["watchlist_panel_html"], unsafe_allow_html=True)
        ...
        with scan_right:
            st.markdown(scan_sayfa_html["universe_panel_html"], unsafe_allow_html=True)
            st.selectbox(
                "Profil",
    ...
    if aktif_sayfa == "🔎 Akıllı Tarama" and st.session_state.tarama_durumu:
        ...
        if not st.session_state.sonuclar:
            ...
            sonuc_kpi_html = tarama_sonuc_kpi_html_paketi_hazirla(
                ...
                st.session_state.boga_sayisi,
                st.session_state.alim_firsati,
            )
            for kpi_col, kpi_html in zip(st.columns(3), sonuc_kpi_html):
                with kpi_col:
            ...
            if not df_sonuc.empty:
                ...
                if st.session_state.izfin_scan_table_focus:
                    ...
                    st.markdown(
                        tarama_odak_meta_html(len(df_sonuc), sonuc_filtresi),
                        unsafe_allow_html=True,
                    )

                    st.markdown(
                        tarama_tablosu_sarmala(izfin_tarama_genis_ozet_html(df_sonuc)),
                        unsafe_allow_html=True,
                    )
                ...
                st.markdown(
                    tarama_tablosu_sarmala(izfin_tarama_tablosu_html(df_sonuc)),
                    unsafe_allow_html=True,
                )
                izfin_sortable_table_js()
...
if aktif_sayfa == "🎯 Projeksiyon & Senaryo":
    ...
    projection_sayfa_html = projection_sayfa_html_paketi_hazirla()
    st.html(projection_sayfa_html["hero_html"])

    if not projection_hazir_mi(st.session_state.tarama_durumu, st.session_state.teknik_paneller):
        st.markdown(
            projection_sayfa_html["empty_html"],
            unsafe_allow_html=True,
        )
        if st.button(
        ...
        with top_right:
            st.markdown(
                projection_sayfa_html["model_note_html"],
                unsafe_allow_html=True,
            )
        ...
        if not proj:
            ...
            st.markdown(
                projection_sayfa_html["model_section_html"],
                unsafe_allow_html=True,
            )
            ...
            senaryo_html = projection_senaryo_html_paketi_hazirla(senaryo, proj)

            st.markdown(
                projection_sayfa_html["scenario_section_html"],
                unsafe_allow_html=True,
            )
            ...
            with al_col:
                st.markdown(senaryo_html["up_html"], unsafe_allow_html=True)

            with sat_col:
                st.markdown(senaryo_html["down_html"], unsafe_allow_html=True)

            st.markdown(senaryo_html["direction_html"], unsafe_allow_html=True)
        ...
        if not kayitlar:
            ...
            with st.expander(f"🗃️ Kapanmış Pozisyon Geçmişi ({len(kapali_df)})", expanded=False):
                if kapali_df.empty:
                    ...
                    kapanmis_html = kapanmis_pozisyon_html_paketi_hazirla(
                        kapanmis_gorunum
                    )
                    st.markdown(kapanmis_html["kpis_html"], unsafe_allow_html=True)
                    st.markdown(kapanmis_html["insight_html"], unsafe_allow_html=True)
                    st.markdown(kapanmis_html["table_html"], unsafe_allow_html=True)
                    if kapanmis_html["reason_summary_html"]:
                        st.markdown(
                            kapanmis_html["reason_summary_html"],
                            unsafe_allow_html=True,
                        )
