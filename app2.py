...
def izfin_movers_render(max_n=5):
    """Büyük Hareketler'i Markdown/özel grid yerine native Streamlit kolonlarıyla çizer."""
    sonuclar = st.session_state.get("sonuclar") or []
    paneller = st.session_state.get("teknik_paneller") or {}
    rows = []
    for sonuc in sonuclar:
    ...
    with st.container(border=True):
        st.html('<div class="iz-card-title">BÜYÜK HAREKETLER</div>')
        if not rows:
            st.info("Akıllı Tarama sonrası büyük hareketler burada görünecek.")
            return
