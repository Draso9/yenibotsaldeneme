...
def aksiyon_rehberi_olustur(nihai_sinyal, teyit_1h):
    ...
    if "YÜKSELİŞ KIRILIMI" in sinyal_metni:
        ...
        ana_metin = "Sistem; varlığın temel verilerinin ve uzun vadeli ana trendinin sağlam olduğunu tespit etti. Kısa vadeli teknik göstergeler düzeltme evresinde olduğu için tek seferde değil, <b>kademeli olarak</b> alım yapılması uygundur."
        ...
        if "TETİK AKTİF" in teyit_metni:
            ...
            alt_not = f'<div style="margin-top: 15px; padding: 10px; background-color: rgba(241, 196, 15, 0.1); border-left: 4px solid #f1c40f; border-radius: 4px;"><b>⏳ HİBRİT TETİK BEKLENİYOR:</b> {teyit_metni} Fiyatın destek dönüşü onaylanana kadar kademeli disiplinle takip edilmelidir.</div>'
        ...
        ana_metin = "Varlık uzun vadeli trendin altında olsa da, normalin çok üzerinde hacim patlaması (%130+) ve güçlü günlük getiri üretti. Yakın takibe alınmalıdır."
        ...
        ana_metin = "Sistem, varlığın ana trendinin (200 SMA) altında olduğunu tespit etti. Sermayeyi koruma disiplini gereği, trend yukarı dönene kadar işlem açılmamalıdır."
        alt_not = '<div style="margin-top: 15px; padding: 10px; background-color: rgba(231, 76, 60, 0.1); border-left: 4px solid #e74c3c; border-radius: 4px;"><b>🛡️ RİSK YÖNETİMİ UYARISI:</b> Ayı trendindeki varlıklarda tetik aranmaz. Sermayenizi koruyun.</div>'
        
    elif "KÂR" in sinyal_metni:
        renk = "#e67e22"
        baslik = "🟠 KÂR REALİZASYONU / ŞİŞKİNLİK"
        ana_metin = "Fiyatın kısa sürede hızla yükseldiğini ve teknik göstergelerin aşırı alım bölgesine girdiğini tespit etti. Kârın bir kısmı realize edilebilir."
        alt_not = '<div style="margin-top: 15px; padding: 10px; background-color: rgba(230, 126, 34, 0.1); border-left: 4px solid #e67e22; border-radius: 4px;"><b>💰 DİSİPLİN ÖNERİSİ:</b> Zirve bölgelerinde yeni alım yapılmaz, kâr cebe atılır.</div>'
        
    else:
        renk = "#95a5a6"
        baslik = "⚪ NÖTR / İZLEMEDE KAL"
        ana_metin = "Sistem sinyalleri şu an belirgin bir yön göstermiyor. Sabırla piyasanın yön seçmesi beklenmelidir."
        alt_not = '<div style="margin-top: 15px; padding: 10px; background-color: rgba(149, 165, 166, 0.1); border-left: 4px solid #95a5a6; border-radius: 4px;"><b>⚖️ BEKLE-GÖR MODU:</b> Net bir trend oluşmamıştır.</div>'
    ...
    if input_val and input_val.strip():
        ...
        if db and st.session_state.user_email:
            try:
                ...
                st.toast("✅ Varlıklar veritabanına eklendi!", icon="💾")
            except:
                pass
    ...
    if input_val and input_val.strip():
        ...
        if degisim:
            if db and st.session_state.user_email:
                try:
                    db.collection("kullanici_listeleri").document(st.session_state.user_email).set({"tickers": st.session_state.custom_tickers})
                    st.toast("✅ Varlık silindi!", icon="🗑️")
                except:
                    pass
            st.session_state.aktif_profil = "Kendi Listem"
            st.session_state.secilen_varliklar = st.session_state.custom_tickers.copy()
...
st.title("📈 Hibrit Portföy Komuta Merkezi")
st.markdown("**Mod:** Ultra Hızlı Paralel Tarama Motoru (Thread-Pool + WAF Korumalı)")
st.markdown("---")
...
with st.sidebar.expander("📋 Varlık Seçimi", expanded=True):
    ...
    st.button("🗑️ Sil", on_click=hisse_sil_callback)
...
# --- TEK BİR HİSSE İÇİN ANALİZ FONKSİYONU (PARALEL ÇALIŞMA İÇİN) ---
def hisseyi_analiz_et(ticker, sektor_getirileri):
    try:
        ...
        sinyal = "Nötr (İzle) ⚖️"
        if breakout_kosulu: sinyal = "YÜKSELİŞ KIRILIMI 🚀"
        elif bugun_kapanis > bb_ust and rsi >= 68: sinyal = "KAR REALİZASYONU 🔴"
        elif bugun_kapanis <= bb_alt and rsi <= 35 and uzun_vade_trend and skor >= 50: sinyal = "KUSURSUZ ALIM 🟢"
        elif rsi <= 40 and uzun_vade_trend and skor >= 50: sinyal = "KADEMELİ ALIM 🔵"
        elif hacim_patlamasi_var and rsi < 50: sinyal = "HACİMLİ TEPKİ 🟡"
        elif skor < 50 or (not uzun_vade_trend and rsi < 50): sinyal = "UZAK DUR! 🛑"
            ...
            try:
                ...
                if not df_1h.empty and len(df_1h) >= 20:
                    ...
                    if (c_1h.iloc[-1] > bb_mid_1h.iloc[-1]) and (v_1h.iloc[-1] > 1.5 * vol_sma_1h.iloc[-1]):
                        ...
                        mikro_teyit = "🔥 TETİK AKTİF: RSI Dip + MACD"
            except:
                pass
...
if tarama_tetiklendi:
    if not selected_tickers:
        ...
        # Sektör referanslarını önceden çek
        sektor_getirileri = {}
        ...
        # --- MULTITHREADING (PARALEL) İŞLEME MOTORU ---
        completed_count = 0
        with ThreadPoolExecutor(max_workers=8) as executor:
            future_to_ticker = {executor.submit(hisseyi_analiz_et, ticker, sektor_getirileri): ticker for ticker in selected_tickers}
            
            for future in as_completed(future_to_ticker):
                completed_count += 1
                progress_val = completed_count / total_tickers
                progress_bar.progress(progress_val)
                progress_text.markdown(f"**⚡ Ultra Hızlı Paralel Tarama (%{int(progress_val * 100)}):** İşlenen Hisse: `{completed_count}/{total_tickers}`")
                
                res, err_ticker = future.result()
                if res:
                    gecici_sonuclar.append(res)
                    if res["Uzun Vade Trend"]: boga_sayisi += 1
                    if res["is_firsat"]: alim_firsati += 1
                else:
                    if err_ticker:
                        basarisi_cekilemeyen_varliklar.append(err_ticker)
...
if st.session_state.tarama_durumu:
    if st.session_state.basarisiz_taramalar:
        st.warning(f"⚠️ Yahoo Finance kaynaklı veri alınamayan varlıklar: **{', '.join(st.session_state.basarisiz_taramalar)}**")
        
    if not st.session_state.sonuclar:
        st.error("❌ Veri alınamadı. Lütfen birkaç saniye sonra tekrar deneyin.")
    else:
        ...
        if "is_firsat" in df_sonuc.columns:
            df_sonuc = df_sonuc.drop(columns=["is_firsat", "Uzun Vade Trend"])
        ...
        if not df_sonuc.empty:
            ...
            if secilen_detay_hisse:
                with st.spinner(f"{secilen_detay_hisse} için grafik yükleniyor..."):
                    ...
                    if not df_grafik.empty:
                        ...
                        fig.update_layout(template='plotly_dark', title=f"{secilen_detay_hisse} - Teknik Paneli", xaxis_rangeslider_visible=False, height=900, margin=dict(l=10, r=10, t=40, b=10), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
                        st.plotly_chart(fig, use_container_width=True)

                        # Sözel Analiz ve Sentez
